...
package joework.input.thirdpersonphysics;
...
import joework.input.thirdpersonphysics.action.BackwardAction;
import joework.input.thirdpersonphysics.action.ForwardAction;
import joework.input.thirdpersonphysics.action.JumpAction;
import joework.input.thirdpersonphysics.action.LeftAction;
import joework.input.thirdpersonphysics.action.RightAction;
import joework.input.thirdpersonphysics.action.StrafeLeftAction;
import joework.input.thirdpersonphysics.action.StrafeRightAction;

/**
 * Class that implement a ThirdPersonHandler with Physics
 * @author joseph
 */
public class ThirdPersonPhysicsHandler extends InputHandler {
    ...
    public static final String PROP_KEY_JUMP = "jumpKey";

    // Default character turnspeed
    ...
    /**
     * Dynamic Physic Node that we are controlling
     * TODO: In future this will be a "Character Target" with
     *       animation movements...
     */
    protected DynamicPhysicsNode targetPhysicsNode;
    ...
    /**
     * internally used boolean for denoting that a strafing action is currently
     * being performed.
     */
    protected boolean nowStrafing;

    /**
     * internally used boolean for denoting that a jumping action is currently
     * being performed
     */
    protected boolean nowJumping;

    // All key input action here
    ...
    protected KeyInputAction actionJump;

    /**
     * Basic constructor for the ThirdPersonPhysicsHandler. Sets all non specified args
     * to their defaults.
     *
     * @param target
     *            the target to move
     ...
     */
    public ThirdPersonPhysicsHandler(DynamicPhysicsNode target, Camera cam) {
     ...
     */
    public ThirdPersonPhysicsHandler(DynamicPhysicsNode target, Camera cam, HashMap<String, Object> props) {
        this.targetPhysicsNode = target;
        this.camera = cam;
    ...
    public void updateKeyBindings(HashMap<String, Object> props) {
        ...
        keyboard.set(PROP_KEY_LEFT, getIntProp(props, PROP_KEY_LEFT, KeyInput.KEY_Q));
        keyboard.set(PROP_KEY_RIGHT, getIntProp(props, PROP_KEY_RIGHT, KeyInput.KEY_E));
        keyboard.set(PROP_KEY_STRAFELEFT, getIntProp(props, PROP_KEY_STRAFELEFT, KeyInput.KEY_A));
        keyboard.set(PROP_KEY_STRAFERIGHT, getIntProp(props, PROP_KEY_STRAFERIGHT, KeyInput.KEY_D));
        keyboard.set(PROP_KEY_JUMP, getIntProp(props, PROP_KEY_JUMP, KeyInput.KEY_SPACE));
    }
    ...
    protected void setActions() {
        actionForward = new ForwardAction( this, 100f );
        actionBack = new BackwardAction( this, 100f );
        actionRight = new RightAction( this, 100f );
        actionLeft = new LeftAction( this, 100f );
        actionStrafeRight = new StrafeRightAction( this, 100f );
        actionStrafeLeft = new StrafeLeftAction( this, 100f );
        actionJump = new JumpAction(this, 100f);
        ...
        addAction( actionJump, PROP_KEY_JUMP, false );
    }
    ...
    public void update(float time) {
        ...
        prevLoc.set(targetPhysicsNode.getLocalTranslation());
        loc.set(prevLoc);

        doInputUpdate(time);
        if (walkingBackwards && walkingForward && !nowStrafing && !nowTurning) {
            targetPhysicsNode.getLocalTranslation().set(prevLoc);
            return;
        }
        targetPhysicsNode.getLocalTranslation().subtract(loc, loc);
        if (!loc.equals(Vector3f.ZERO)) {
            ...
            float actAngle = 0;
            targetPhysicsNode.getLocalRotation().getRotationColumn(2, calcVector);
            if (upVector.y == 1) {
            ...
            if (nowStrafing) {
                faceAngle = actAngle;
                prevRot.set(targetPhysicsNode.getLocalRotation());
            }
            targetPhysicsNode.getLocalRotation().fromAngleNormalAxis(-(faceAngle - FastMath.HALF_PI), upVector);
            targetPhysicsNode.getLocalRotation().getRotationColumn(2, calcVector).multLocal(distance);
            ...
            if (nowStrafing) {
                if (!strafeAlignTarget && cameraAlignedMovement) {
                    ...
                    targetPhysicsNode.getLocalRotation().fromAngleNormalAxis(-faceAngle, upVector);
                } else {
                    targetPhysicsNode.getLocalRotation().set(prevRot);
                    faceAngle = oldFace;
                }
            }
            ...
            if (lockBackwards && walkingBackwards && !nowStrafing)
                targetPhysicsNode.getLocalTranslation().subtractLocal(calcVector);
            else if (rotateOnly && nowTurning && !walkingBackwards && !walkingForward)
                ; // no translation
            else
                targetPhysicsNode.getLocalTranslation().addLocal(calcVector);
        }
    }

    private void doInputUpdate(float time) {
        super.update(time);
    }
    ...
    public DynamicPhysicsNode getTarget() {
        return targetPhysicsNode;
    }

    public void setTarget(DynamicPhysicsNode target) {
        this.targetPhysicsNode = target;
    }
    ...
    /**
     * Internal method used to let the handler know that the target is currently
     * jumping (via use of the jump key.)
     *
     * @param jump
     */
    ...
    /**
     * @return true if last update of handler included a strafing action.
     */
    ...
    /**
     * @return true if last update of handler included a jumping action.
     */
